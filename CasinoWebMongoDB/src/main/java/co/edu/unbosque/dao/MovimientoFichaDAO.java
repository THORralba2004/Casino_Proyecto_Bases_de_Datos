...
public class MovimientoFichaDAO implements ICrud {
        ...
        try {
            ...
            if (resultadoInventario != null && resultadoInventario.startsWith("2")) {
                ...
                if (listaMovimientos != null) {
                    movimiento.setIdMov(String.valueOf(obtenerUltimoId() + 1)); // Simular ID
    ...
    public String actualizar(Object id, Object registro) {
        ...
        try {
            ...
            MovimientoFichaDTO movimientoViejo = consultarPorId((String) id);
            ...
            if (movimientoViejo != null) {
                ...
                if (resultadoRevertir != null && resultadoRevertir.startsWith("2")) {
                    ...
                    if (resultadoAplicar != null && resultadoAplicar.startsWith("2")) {
                        ...
                        int respuesta = MovimientoFichaJSON.updateJSON((String) id, movimientoNuevo);
                        mensaje = String.valueOf(respuesta);
                        ...
                        if (listaMovimientos != null) {
                            for (int i = 0; i < listaMovimientos.size(); i++) {
                                if (listaMovimientos.get(i).getIdMov().equals(id)) {
    ...
    public String eliminar(Object id) {
        ...
        try {
            ...
            MovimientoFichaDTO movimiento = consultarPorId((String) id);
            if (movimiento != null) {
                ...
                if (resultadoInventario != null && resultadoInventario.startsWith("2")) {
                    ...
                    int respuesta = MovimientoFichaJSON.deleteJSON((String) id);
                    mensaje = String.valueOf(respuesta);
                    System.out.println("✅ Movimiento eliminado. Respuesta: " + mensaje);
                    
                    // Actualizar la lista local
                    if (listaMovimientos != null) {
                        listaMovimientos.removeIf(m -> m.getIdMov().equals(id));
                    }
                } else {
                    mensaje = resultadoInventario;
    ...
    public MovimientoFichaDTO consultarPorId(String id) {
        try {
    ...
    public ArrayList<MovimientoFichaDTO> filtrarPorBoveda(String idInventario) {
        if (listaMovimientos == null) {
            consultar();
        }
        ArrayList<MovimientoFichaDTO> filtrados = new ArrayList<>();
        if (listaMovimientos != null) {
            for (MovimientoFichaDTO movimiento : listaMovimientos) {
                if (movimiento.getIdInventario().equals(idInventario)) {
                    filtrados.add(movimiento);
                }
            }
        }
    ...
    private int obtenerUltimoId() {
        ...
        for (MovimientoFichaDTO movimiento : listaMovimientos) {
            try {
                int idActual = Integer.parseInt(movimiento.getIdMov());
                if (idActual > maxId) {
                    maxId = idActual;
                }
            } catch (NumberFormatException e) {
                // Si el ID no es numérico, ignorar para este cálculo
            }
        }
        return maxId;
    }
    ...
    public boolean verificarEgresoPosible(String idInventario, int cantidad) {
        try {
            InventarioFichaDAO inventarioDAO = new InventarioFichaDAO();
    ...
    public int obtenerSaldoBoveda(String idInventario) {
        try {
            InventarioFichaDAO inventarioDAO = new InventarioFichaDAO();
